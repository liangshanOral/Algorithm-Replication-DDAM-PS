...
class OIM(autograd.Function):
    ...
    def forward(ctx, inputs, targets, lut, cq, header, momentum):
        ...
        outputs_labeled = inputs.mm(lut.t()) #有标签样本输出
        outputs_unlabeled = inputs.mm(cq.t()) # 无标签样本输出
...
class OIMLoss(nn.Module):
    def __init__(self, num_features, num_pids, num_cq_size, oim_momentum, oim_scalar):
        ...
        self.num_pids = num_pids #有标签的身份个数
        self.num_unlabeled = num_cq_size #循环队列的大小，用于未标记样本
