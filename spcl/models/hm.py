...
class HybridMemory(nn.Module):
    ...
    def forward(self, inputs, indexes, is_source=True):
        ...
        inds = (indexes>=0) #获取有效标签
        indexes = indexes[inds]
        for i in range(len(indexes)):
            if (indexes[i] == 5554) and is_source:
                #说明这个人没有标注
                indexes[i] = self.source_classes-1
        inputs = inputs[inds.unsqueeze(1).expand_as(inputs)].view(-1, self.num_features) #获得有效样本特征

        inputs = hm(inputs, indexes, self.features, self.momentum).float().cuda() #通过内存进行更新
        ...
        sim = torch.zeros(labels.max()+1, B).float().cuda() #相似度矩阵
    ...
    def forward_prob(self, inputs, indexes=None, is_source=True):
        ...
        outputs = inputs.mm(self.features.t()) # 计算输入特征向量与存储的特征向量之间的点积
        return outputs